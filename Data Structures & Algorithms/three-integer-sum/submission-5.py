        ...
        while i < (size - 2): # check up until size - 2
            ...
            while j < k: # sliding window
                ...
                if target == add: # then add the nums to result
                    ...
                    j += 1
                    k -= 1
                    # these because the closer they are it's more likely they can find another match (converges to the more similar number to the target)
...
# Time Complexity
# - Sorted -> O(nlogn)
# - Loop i + Loop sliding window O(n2)
# O(n2)

# Space Complexity
# O(1) we change nothing in the arr